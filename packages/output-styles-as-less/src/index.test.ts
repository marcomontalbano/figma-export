...
describe('style output as less', () => {
    ...
    it('should not print anything if style is not visible', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(path.resolve('output-folder', '_variables.less'), '');
    });
    ...
    it('should be able to change the filename, the extension and output folder', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledOnce();
        expect(fs.writeFileSync).toHaveBeenCalledWith(path.resolve('output-folder', '_figma-styles.less'), '');
    });
