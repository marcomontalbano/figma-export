...
export default ({
  ...
  getExtension = () => 'JSON',
  getFilename = () => 'base',
  getVariableName = (style, descriptor) =>
    `${kebabCase(style.name).toLowerCase()}${descriptor != null ? `-${descriptor}` : ''}`,
  ...
  return async (styles) => {
    ...
    fs.writeFileSync(
      path.resolve(filePath, `${getFilename()}.${extension.toLowerCase()}`),
