...
const getFile = async (
  ...
  const {
    data: { document = null, styles = null } = {},
  } = await client
    .file(options.fileId, {
...
const getPagesFromDocument = (
  ...
  return document.children.filter((node): node is Figma.Canvas => {
    return (
      node.type === 'CANVAS' &&
  ...
  return pages
    .map((page) => ({
      ...
      components: getComponents(
        page.children as readonly FigmaExport.ComponentNode[],
        options,
      ),
    }))
