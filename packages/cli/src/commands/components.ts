...
export const addComponents = (prog: Sade, spinner: Ora) =>
  prog
    ...
    .option(
      '-i, --ids',
      'Export only these node IDs (`--page` is always ignored when set)',
    )
    .option(
      '-p, --page',
    ...
    .option(
      '-t, --types',
      'Node types to be exported (COMPONENT or INSTANCE)',
      'COMPONENT',
    )
    .option(
      '--fileVersion',
    ...
    .example(
      'components fzYhvQpqwhZDUImRz431Qo -O @figma-export/output-components-as-svg',
    )
    .example(
      'components fzYhvQpqwhZDUImRz431Qo -O @figma-export/output-components-as-svg -t COMPONENT -t INSTANCE -o dist',
    )
    .example(
      'components fzYhvQpqwhZDUImRz431Qo -O @figma-export/output-components-as-svg -o dist -i 54:22 -i 138:52',
    )
    .action(
      async (
        fileId,
        { fileVersion, concurrency, retries, output, ...opts },
      ) => {
        type IncludeTypes =
          FigmaExport.ComponentsCommandOptions['includeTypes'];
        ...
        spinner.info(
          `Exporting ${fileId} with [${transformer.join(', ')}] as [${outputter.join(', ')}]`,
        );
        ...
        figmaExport
          .components({
            ...
            includeTypes: types,
            transformers:
              await requirePackages<FigmaExport.StringTransformer>(transformer),
            outputters: await requirePackages<FigmaExport.ComponentOutputter>(
              outputter,
              { output },
            ),
